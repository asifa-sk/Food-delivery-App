...
import { readFavoriteIds, readFavoriteItems, writeFavoriteIds, writeFavoriteItems } from "../utils/favoritesStorage";
...
export default function FavoritesPage() {
    ...
    const load = async () => {
      if (user && user.id) {
        try {
          ...
          writeFavoriteIds(user.id, Array.isArray(favs) ? favs.map((r) => r.id) : []);
          setLoading(false);
          return;
        } catch (e) {
  ...
  useEffect(() => {
    setFavoriteItems(readFavoriteItems(user?.id));
  }, [user?.id]);
        ...
        {loading ? (
          ...
          <>
                    ...
                    {favoriteItems.map((it) => (
                      <div key={it.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-brand-600 font-bold mt-1">Rs. {it.price}</div>
                        </div>
                        <button onClick={() => { setFavoriteItems(prev => { const upd = prev.filter(p => String(p.id)!==String(it.id)); writeFavoriteItems(user?.id, upd); return upd; }) ; if (typeof showToast==='function') showToast('Removed item from favorites',{type:'success'}) }} className="ml-auto text-red-500">Remove</button>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="text-center py-24">
              ...
              <button onClick={() => navigate("/home")} className="mt-5 px-6 py-3 bg-brand-500 hover:bg-brand-600 text-white font-bold rounded-2xl transition">
                Explore Restaurants
              </button>
            </div>
          </>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {favoriteRestaurants.map((r) => (
              ...
              >
                <div className="relative h-36 bg-gradient-to-br from-brand-100 to-accent-200 flex items-center justify-center overflow-hidden">
                  {r.imageUrl ? (
                ...
                <div className="p-4">
                  <h3 className="font-bold text-gray-900 group-hover:text-brand-600 transition">{r.name}</h3>
                  {r.cuisineType && (
                    <p className="text-brand-500 text-xs font-semibold mt-0.5">{r.cuisineType}</p>
                  )}
